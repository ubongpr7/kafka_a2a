...
def test_is_owner_claim_allows_owner_override_without_owner_id() -> None:
    secret = "owner-claim-test-secret"
    config = JwtBearerConfig(secret=secret, algorithms=["HS256"], include_claims=True)
    now = datetime.now(timezone.utc)
    access_token = jwt.encode(
        {
    ...
    context = build_agent_auth_context(token=access_token, jwt=config)

    assert context.is_workspace_owner
    assert context.has_permission("manage_agent_settings")


def test_non_owner_without_permission_remains_denied() -> None:
    secret = "non-owner-test-secret"
    config = JwtBearerConfig(secret=secret, algorithms=["HS256"], include_claims=True)
    now = datetime.now(timezone.utc)
    access_token = jwt.encode(
        {
    ...
    assert not context.is_workspace_owner
    assert not context.has_permission("manage_agent_settings")
